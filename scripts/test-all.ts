...
async function testPharmacyCRUD() {
  ...
  try {
    ...
    // 5.4 اختبار فك تشفير كلمة المرور
...
async function testOrders() {
  log("\n🛒 اختبار الطلبات...", "info");
  try {
    // 7.1 البحث عن منتج وصيدلية للاختبار
    const [product] = await db
      .select()
      .from(products)
    ...
    // 7.2 اختبار التحقق من حد الائتمان
    const orderTotal = 1000; // قيمة وهمية
    ...
    // 7.6 التحقق من الخصم وتحديث الدين
    const [updatedProduct] = await db
      .select()
      .from(products)
      .where(eq(products.id, product.id))
    ...
    // 7.7 تغيير حالة الطلب إلى "مقبول"
    await db
      .update(orders)
      .set({ status: "approved" })
    ...
    // 7.8 إعادة المخزون والدين (تنظيف)
    await db
      .update(products)
...
async function testReviews() {
  ...
  try {
    ...
    // 9.2 حساب متوسط التقييم (محاكاة تحديث المنتج)
    // نأخذ متوسط كل التقييمات للمنتج
...
async function testNotifications() {
  ...
  try {
    ...
    // 10.2 تحديث حالة الإشعار إلى مقروء
    await db
